...
#
# [3 juin 2025] v 2.6.10
# - CORRECTION CRITIQUE: Fonction validate_configuration passait la configuration complète au lieu du nom de serveur
# - Correction de l'appel validate_server_name("$server_config") en validate_server_name("$server_name")
# - Simplification de la fonction validate_server_name pour ne traiter que le nom de serveur extrait
# - AMÉLIORATION: Refonte complète du rapport de validation de configuration
# - Séparation des serveurs accessibles et inaccessibles dans le rapport HTML
# - Affichage correct des noms de serveurs (extraction de "mini.nojo.fr" depuis "serge@mini.nojo.fr")
# - Section dédiée aux serveurs inaccessibles avec raisons détaillées (DNS, ping, SSH)
# - Statistiques corrigées distinguant serveurs configurés, accessibles et inaccessibles
# - SUPPRESSION: Abandon du support macOS/Darwin dans le script
# - Suppression de la fonction detect_os et des adaptations spécifiques macOS
# - Suppression des commandes df adaptées pour macOS dans check_disk_space et show_sorted_usage
# - Simplification du code avec support Linux uniquement pour une meilleure maintenabilité
# - Les serveurs macOS devront utiliser une solution de monitoring dédiée
#
...
validate_configuration() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        # Ajouter le serveur à la liste (garder la ligne complète)
        servers+=("$line")
    done < "$SERVER_LIST"
    ...
    for server_config in "${servers[@]}"; do
        ...
        server_info=$(extract_server_info "$server_config")
        server_name=$(echo "$server_info" | cut -d' ' -f1)
        ssh_user=$(echo "$server_info" | cut -d' ' -f2)
        
        echo -e "\nTest du serveur: $server_name (config: $server_config)"
        
        # Vérification de la syntaxe du nom
        if ! validate_server_name "$server_name" "warn"; then
            echo "  AVERTISSEMENT: Nom de serveur potentiellement invalide: $server_name"
            warnings=$((warnings + 1))
            server_status["$server_config"]="warning"
        else
            echo "  OK: Syntaxe du nom de serveur valide"
            server_status["$server_config"]="ok"
        fi
        
        # Test de résolution DNS
        if ! host "$server_name" >/dev/null 2>&1; then
            echo "  ERREUR: Impossible de résoudre le nom d'hôte: $server_name"
            server_status["$server_config"]="error"
            continue
        else
        ...
        if ping -c 1 -W 2 "$server_name" >/dev/null 2>&1; then
            ...
            echo "  AVERTISSEMENT: Serveur non accessible par ping: $server_name"
            warnings=$((warnings + 1))
            server_status["$server_config"]="warning"
        fi
        ...
        if $ssh_cmd "echo test" >/dev/null 2>&1; then
            ...
            echo "  ERREUR: Impossible de se connecter en SSH à $server_name"
            critical_errors=$((critical_errors + 1))
            server_status["$server_config"]="error"
            continue
        fi
            ...
            if [[ ! -z "$partitions" ]]; then
                ...
                if [[ "$partitions" =~ , ]]; then
                    ...
                    for part in "${PART_ARRAY[@]}"; do
                        ...
                        if $ssh_cmd "df -h $part" >/dev/null 2>&1; then
                            echo "    OK: Partition $part accessible"
                        else
                            echo "    ERREUR: Partition $part introuvable ou inaccessible sur $server_name"
                            critical_errors=$((critical_errors + 1))
                            server_status["$server_config"]="error"
                        fi
                    done
                    ...
                    # Vérifier si la partition existe
                    if $ssh_cmd "df -h $partitions" >/dev/null 2>&1; then
                        echo "    OK: Partition $partitions accessible"
                    else
                        echo "    ERREUR: Partition $partitions introuvable ou inaccessible sur $server_name"
                        critical_errors=$((critical_errors + 1))
                        server_status["$server_config"]="error"
                    fi
                fi
...
<head>
    ...
    <style>
        ...
        .inaccessible-list { background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; }
        .inaccessible-list ul { margin: 10px 0; }
        .inaccessible-list li { margin: 5px 0; }
    </style>
</head>
<body>
    ...
    <h2>Résultats des tests de serveurs accessibles</h2>
    <table>
        <tr>
            <th>Serveur</th>
            <th>Statut</th>
...
EOF
    ...
    # Créer deux listes : serveurs accessibles et inaccessibles
    declare -a accessible_servers
    declare -a inaccessible_servers
    
    # Séparer les serveurs accessibles des inaccessibles
    for server_config in "${servers_ref[@]}"; do
        # Extraire le vrai nom du serveur (sans utilisateur)
        server_info=$(extract_server_info "$server_config")
        server_name=$(echo "$server_info" | cut -d' ' -f1)
        ssh_user=$(echo "$server_info" | cut -d' ' -f2)
        
        # Tester l'accessibilité basique (ping + SSH)
        ...
        # Classer le serveur
        if $is_accessible; then
            accessible_servers+=("$server_config")
        else
            inaccessible_servers+=("$server_config")
        fi
    done

    # Traiter les serveurs accessibles pour le tableau principal
    for server_config in "${accessible_servers[@]}"; do
        ...
        status_text="OK"
        if [[ "${status_ref[$server_config]}" == "warning" ]]; then
            status_class="warning"
            status_text="ATTENTION"
            warning_count=$((warning_count + 1))
        elif [[ "${status_ref[$server_config]}" == "error" ]]; then
            status_class="error"
            status_text="ERREUR"
        ...
        # Ajouter la ligne du serveur au tableau (utiliser server_name au lieu de server_config)
        cat >> "$report_file" <<EOF
...
EOF
    ...
    # Fermer le tableau principal
    echo "    </table>" >> "$report_file"
    
    # Ajouter la section des serveurs inaccessibles si nécessaire
    if [ ${#inaccessible_servers[@]} -gt 0 ]; then
        cat >> "$report_file" <<EOF
    
    <div class="inaccessible-list">
        <h2>Serveurs inaccessibles</h2>
        <p>Les serveurs suivants n'ont pas pu être testés car ils sont inaccessibles (ping ou SSH en échec) :</p>
        <ul>
EOF
        for server_config in "${inaccessible_servers[@]}"; do
            server_info=$(extract_server_info "$server_config")
            server_name=$(echo "$server_info" | cut -d' ' -f1)
            
            # Déterminer la raison de l'inaccessibilité
            local reason="Ping et SSH en échec"
            if ping -c 1 -W 2 "$server_name" >/dev/null 2>&1; then
                reason="SSH en échec (ping OK)"
            elif ! host "$server_name" >/dev/null 2>&1; then
                reason="Résolution DNS en échec"
            else
                reason="Ping en échec"
            fi
            
            echo "        <li><strong>$server_name</strong> - $reason (configuration: $server_config)</li>" >> "$report_file"
        done
...
EOF
    ...
    local accessible_count=${#accessible_servers[@]}
    local inaccessible_count=${#inaccessible_servers[@]}
    
    # Ajouter le résumé corrigé
    cat >> "$report_file" <<EOF
    
    <h2>Résumé</h2>
    <div class="summary">
        <p>Serveurs configurés au total: $total_servers</p>
        <p>Serveurs accessibles testés: $accessible_count</p>
        <p>Serveurs inaccessibles: $inaccessible_count</p>
        <p>Erreurs dans la configuration: $error_count</p>
...
validate_server_name() {
    local server="$1"  # Maintenant c'est juste le nom de serveur
