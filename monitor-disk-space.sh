...
# 2. Analyse du fichier de serveurs (server-disk-space.conf)
...
# 5. Envoi des emails selon le niveau d'alerte
# 6. Envoi des rapports consolidés (si demandé)
# 7. Rotation des logs (si activée)
# 8. Nettoyage
#
#--------------------------------------------------------------
...
check_disk_space() {
    ...
    for partition in $partitions; do
        ...
        # Utiliser une commande SSH propre pour obtenir les informations
        local ssh_output=$(ssh "$server" "df -h $partition" 2>/dev/null | grep -v "Filesystem")
        ...
        if [ $? -eq 0 ] && [ ! -z "$ssh_output" ]; then
            ...
            local usage_str=$(echo "$ssh_output" | awk '{print $5}')
            local mount_point=$(echo "$ssh_output" | awk '{print $6}')
    ...
    for partition in $partitions; do
        local inode_info=$(ssh "$server" "df -i $partition" | grep -v Filesystem)
        ...
        if [ $? -eq 0 ] && [ ! -z "$inode_info" ]; then
            ...
            # Extraire uniquement les chiffres du pourcentage
            local iusage_percent=$(echo "$iusage_str" | grep -o '[0-9]*')
            local mount_point=$(echo "$inode_info" | awk '{print $6}')
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # Ajouter le serveur à notre tableau
    servers+=("$line")
done <"$SERVER_LIST"
